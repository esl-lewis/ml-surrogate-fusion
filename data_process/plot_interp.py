...
MAGC_file = "../JET_EFIT_magnetic/99070_MAGC.csv"
EFIT_file = "../JET_EFIT_magnetic/99070_EFIT.csv"
INTERP_file = "../JET_EFIT_magnetic/interpolated_99070.csv"


interp_df = pd.read_csv(INTERP_file)

# print(interp_df.columns)

interp_plot = interp_df[["Time", "BPME_17"]]
# interp_plot = interp_plot.set_index(["Time"])

mag_df = pd.read_csv(MAGC_file)
mag_plot = mag_df[["Time", "BPME_17"]]
print(mag_plot.head(3))
...
# plt.plot(merge_df["Time"], merge_df["BPME_17"])
...
plt.scatter(
    ...
    color="green",
    marker="x",
    label="Probe 17 interpolated (onto EFIT timestep)",
)
...
plt.plot(
    ...
    color="red",
    marker=".",
    label="Probe 17 raw (MAGC)",
)

"""
for column_val in mag_df.columns:
    if column_val.startswith("BPME"):
        plt.plot(
